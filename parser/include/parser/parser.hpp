...
public:
    ...
    Holder<ast::IStatement> ParseContinueStatement() {
        return MakeHolder<ast::ContinueStatement>(std::move(*scanner->NextTokenAssert(Token::CONTINUE)));
    }
    ...
    Holder<ast::FunctionDefinitionExpression> ParseFunctionDefinitionExpression() {
        ...
        if (scanner->NextTokenLookahead()->token == Token::IDENTIFIER) {
            name = MakeHolder<ast::IdentifierLiteral>(std::move(*scanner->NextToken()));
        }
        auto left_par = scanner->NextTokenAssert(Token::LEFT_PAR);
    ...
    Holder<ast::IExpression> ParsePostfixExpression() {
        ...
        while (true) {
            ...
            if (auto token = scanner->NextToken(); tokens_set.find(token->token) != tokens_set.end()) {
                expr = MakeHolder<ast::PostfixExpression>(std::move(expr), std::move(*token));
            } else if (token->token == Token::LEFT_PAR) {
                auto args = ParseCallOrSubscriptArguments(Token::RIGHT_PAR);
                expr = MakeHolder<ast::FunctionCallExpression>(std::move(expr), std::move(*token), std::move(args), std::move(*scanner->NextTokenAssert(Token::RIGHT_PAR)));
            } else if (token->token == Token::LEFT_BRACKET) {
                auto args = ParseCallOrSubscriptArguments(Token::RIGHT_BRACKET);
                expr = MakeHolder<ast::SubscriptExpression>(std::move(expr), std::move(*token), std::move(args), std::move(*scanner->NextTokenAssert(Token::RIGHT_BRACKET)));
            } else if (token->token == Token::DOT) {
                expr = MakeHolder<ast::MemberAccessExpression>(std::move(expr), std::move(*token), MakeHolder<ast::IdentifierLiteral>(std::move(*scanner->NextTokenAssert(Token::IDENTIFIER))));
            } else {
                mark.Apply();
                break;
            }
        ...
        while (true) {
            ...
            if (auto token = scanner->NextToken(); tokens_set.find(token->token) != tokens_set.end()) {
                if constexpr (associativity == Associativity::LEFT) {
                    expr = MakeHolder<ast::BinaryExpression>(std::move(expr), std::move(*token), (this->*next)());
                } else {
                    expr = MakeHolder<ast::BinaryExpression>(std::move(expr), std::move(*token), ParseBinaryExpression<next, allow_newline_before_op, associativity, tokens...>());
                }
            } else {
                mark.Apply();
                break;
